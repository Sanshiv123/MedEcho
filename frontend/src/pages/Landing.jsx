...
// MedEcho marketing landing page — the public-facing entry point.
//
// Sections:
//   1. Navbar        — sticky top bar with logo, Log In, Sign Up
//   2. Hero          — animated MedEchoLogo + tagline + CTA
//   3. Features      — 6-card grid explaining key capabilities
//   4. How it works  — 4-step numbered flow
//   5. Portals       — 3-column breakdown by role (clinician/physician/patient)
//   6. CTA           — bottom call to action
//   7. Footer        — logo + tagline
//
// Role selection is handled via a modal (RoleModal) that appears when
// the user clicks any CTA button. Selecting a role navigates to the
// appropriate portal (/clinician, /physician, /patient/demo).
//
// Theme-aware — adapts colors for light and dark mode by reading
// the data-theme attribute on document.body. A MutationObserver
// triggers re-renders when the theme changes.
...
/** Returns true if the current theme is dark (default) */
...
/**
 * Overlay modal for selecting a portal role.
 *
 * Appears when the user clicks any CTA button. Clicking outside or
 * "Cancel" closes it. Selecting a role navigates to the portal.
 *
 * Props:
 *   onClose  - Called when the modal should close
 *   onSelect - Called with the role key ("clinician", "physician", "patient")
 */
function RoleModal({ onClose, onSelect }) {
  ...
  return (
    ...
    >
      ...
      >
        <p style={{ fontFamily: "Syne, sans-serif", fontSize: 20, fontWeight: 700, color: textPrimary, marginBottom: 6 }}>
          Open portal as…
        </p>
          ...
          {roles.map(({ key, label, icon, desc }) => (
            <button
              ...
              onMouseEnter={e => {
                ...
                e.currentTarget.style.background = dark ? "rgba(127,119,221,0.1)" : "rgba(127,119,221,0.06)";
              }}
              onMouseLeave={e => {
                e.currentTarget.style.borderColor = border;
                e.currentTarget.style.background = dark ? "rgba(255,255,255,0.04)" : "rgba(127,119,221,0.04)";
              }}
            >
        ...
        <button
          ...
          style={{
            ...
            border: `1px solid ${border}`,
            borderRadius: 10,
            fontSize: 13, color: textMuted, cursor: "pointer"
          }}
        >
          Cancel
        </button>
...
export default function Landing() {
  ...
  // Re-render when theme changes so colors update immediately
  useEffect(() => {
  ...
  // ---------------------------------------------------------------------------
  const dark = isDark();
  const bg          = dark ? "linear-gradient(135deg, #060E1A 0%, #0A1628 100%)" : "linear-gradient(160deg, #EEF4FF 0%, #E8F0FF 50%, #F0F4FF 100%)";
  const textPrimary = dark ? "#E2EAF8"                   : "#0F172A";
  const textMuted   = dark ? "#4A6A8A"                   : "#64748B";
  ...
  const handleRole = (role) => {
    setShowModal(false);
    // Patient portal requires a patient ID — use /demo for the landing page preview
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, fontFamily: "DM Sans, sans-serif", color: textPrimary }}>
      ...
      {/* Background ambient orbs — decorative blurred circles */}
